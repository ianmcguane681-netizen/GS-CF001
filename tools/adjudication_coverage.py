...
def main() -> int:
    ...
    for index, record in enumerate(records, start=1):
        ...
        try:
            ...
            if budget.record_deferral(index, len(records)):
                print(f"\nSTOPPING: {budget.reason}")
                break
            time.sleep(DEFERRAL_BACKOFF_SECONDS)
            continue
    ...
    if deferred or budget.exhausted:
        ...
        unattempted = len(records) - len(results) - len(deferred)
        _write(
            {
                "note": (
                    ...
                    + (
                        f" The walk stopped early -- {budget.reason} -- leaving "
                        f"{unattempted} record(s) not reached at all."
                        if budget.exhausted
                        else ""
                    )
                ),
                "stopped_early": budget.reason or None,
                "not_reached": unattempted if budget.exhausted else 0,
        ...
        if budget.exhausted:
            print(f"{unattempted} record(s) were not reached; the walk stopped at the wall.")
        return 2
