"""What a sector has to declare before this engine can study it.

The shape is deliberately narrow. A pack supplies vocabulary and named mechanisms; it
does not supply logic, thresholds, or gate outcomes. A sector that could set its own
...
from verification.rules import contains_any
...
@dataclass(frozen=True, slots=True)
class SectorPack:
    """One sector's vocabulary, mechanisms and sources.
    ...
    cross-sector match impossible as well as forbidden.
    """
    ...
    # Retrieval is correctly per-sector: connectors are the socket, not the weld.
    ...
    def __post_init__(self) -> None:
        ...
        if undefined:
            raise ValueError(f"{self.sector_id}: mechanisms declared but not defined: {undefined}")
        if self.default_mechanism in {rule.mechanism_id for rule in self.mechanism_rules}:
            raise ValueError(
                f"{self.sector_id}: the fallback {self.default_mechanism!r} is also a rule "
